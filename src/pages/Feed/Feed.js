...
class Feed extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:8080/graphql", {
          ...
          userStatus{
			...
			.then(resData => {
				this.setState({ status: resData.data.userStatus.message });
			})
  ...
  statusUpdateHandler = event => {
    ...
    fetch('http://localhost:8080/graphql', {
      ...
      .then(resData => {
        ...
        this.setState({ status: resData.data.updateStatus.message });
      })
      .catch(this.catchError);
  };
  ...
  finishEditHandler = postData => {
    ...
    .then(uploadRes => {
      const imageUrl = uploadRes.filePath;
      let graphqlQuery;
      if(this.state.editPost){
        graphqlQuery = {
					query: `
        ...
        const post = {
          ...
          let updatedPostsTotal = prevState.totalPosts;
